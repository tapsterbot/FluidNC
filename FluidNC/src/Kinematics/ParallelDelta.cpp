...
namespace Kinematics {
    ...
    bool ParallelDelta::cartesian_to_motors(float* target, plan_line_data_t* pl_data, float* position) {
        ...
        transform_cartesian_to_motors(last_angle, position);
        //if (!calc_ok) {
        ...
        transform_cartesian_to_motors(motor_angles, target);
        //if (!calc_ok) {
        ...
        for (uint32_t segment = 1; segment <= segment_count; segment++) {
            ...
            transform_cartesian_to_motors(motor_angles, seg_target);
    ...
    void ParallelDelta::transform_cartesian_to_motors(float* motors, float* cartesian) {
